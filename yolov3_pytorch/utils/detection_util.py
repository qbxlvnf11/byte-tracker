...
def draw_id_boxes(img, id_list, box_list, class_list, save_path, img_id):
	...
	for id_num, box, class_name in zip(id_list, box_list, class_list):
		# xyxy format
		x1, y1, x2, y2 = list(map(int, box))
		
		id_class = str(id_num) + ' / ' + class_name
		
		t_size = cv2.getTextSize(id_class, cv2.FONT_HERSHEY_PLAIN, 1 , 1)[0]
		cv2.rectangle(img, (x1, y1), (x2, y2), color, thickness)
		cv2.rectangle(img, (x1, y1), (x1+t_size[0]+3, y1+t_size[1]+4), color, -1)
		cv2.putText(img, id_class, (x1, y1+t_size[1]+4), cv2.FONT_HERSHEY_PLAIN, 1, [255,255,255], 1)
